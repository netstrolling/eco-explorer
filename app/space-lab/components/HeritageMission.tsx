...
import { Site, checkTimeSync, localizeEra } from '../lib/heritage';
  ...
  return (
    <div className="sl-modal-backdrop" onClick={onClose}>
      <div className="sl-modal sl-modal-heritage" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sl-her-badge">{localizeEra(site.era, lang)} · {site.theme}</div>
        ...
        {result === 'ok' || alreadyDone ? (
          <div className="sl-her-success">
            ...
            <strong>{t({ ko: '“{n}” 수집!', en: 'Collected “{n}”!' }, { n: site.artifact.name })}</strong>
            <p style={{ fontSize: 13, color: '#6b5836' }}>{t({ ko: '도감에 기록되었습니다.', en: 'Saved to your log.' })}</p>
            <button className="sl-btn primary" onClick={onClose}>{t({ ko: '확인', en: 'OK' })}</button>
          </div>
        ) : (
          <>
            ...
            {site.mission.kind === 'timesync' && (
              <input className="sl-input sl-input-light" placeholder={t({ ko: '예: 14:08', en: 'e.g. 14:08' })} value={input}
                onChange={(e) => { setInput(e.target.value); setResult('none'); }} />
            )}

            {result === 'no' && <p style={{ color: '#b3261e', fontSize: 13, marginTop: 10 }}>{t({ ko: '아쉬워요! 다시 시도해 보세요.', en: 'Not quite! Try again.' })}</p>}

            <div className="sl-btn-row" style={{ justifyContent: 'center', marginTop: 14 }}>
              <button className="sl-btn primary" onClick={site.mission.kind === 'quiz' ? submitQuiz : submitTime}>{t({ ko: '제출', en: 'Submit' })}</button>
              <button className="sl-btn" onClick={onClose}>{t({ ko: '닫기', en: 'Close' })}</button>
            </div>
          </>
        )}
      </div>
