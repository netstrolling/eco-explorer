...
import {
  GALDA, PLANETS, START_PRESETS, START_AU, PLANET_EN,
...
function JourneyView({ setup, onBgChange, onReset }: { setup: SetupResult; onBgChange: (bg: string) => void; onReset: () => void }) {
  const { t } = useI18n();
  const pn = (pl: Planet) => t({ ko: pl.name, en: PLANET_EN[pl.key]?.name ?? pl.name });
  const mTitle = (pl: Planet) => t({ ko: pl.mission?.title ?? '', en: PLANET_EN[pl.key]?.mission?.title ?? pl.mission?.title ?? '' });
  const mDesc = (pl: Planet) => t({ ko: pl.mission?.desc ?? '', en: PLANET_EN[pl.key]?.mission?.desc ?? pl.mission?.desc ?? '' });
    ...
    <>
      <div className="sl-panel" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <span style={{ color: 'var(--sl-muted)', fontSize: 12, marginLeft: 6 }}>{t({ ko: '출발', en: 'From' })}: {setup.startName}</span>
        </div>
      ...
      <div className="sl-statusbar">
        {t({ ko: '현재 위치', en: 'Now' })}: <strong>{pn(orbit)}</strong> {t({ ko: '궤도', en: 'orbit' })} ({au.toFixed(2)} AU)…{' '}
        {p < 1 ? <>{t({ ko: '갈다까지', en: 'to Galda' })} {formatMeters(distToGalda)} · <span className="sl-gravity">{t({ ko: '중력 가속', en: 'gravity' })} ×{grav.toFixed(1)}</span></> : t({ ko: '🌍 갈다 권역 진입!', en: '🌍 Entering Galda’s zone!' })}
      </div>
      ...
      <div className="sl-panel">
        ...
        <div className="sl-stat-grid">
          <div className="sl-stat"><div className="v">{au.toFixed(1)}</div><div className="k">{t({ ko: '현재 AU', en: 'Current AU' })}</div></div>
          <div className="sl-stat"><div className="v">{Math.round(p * 100)}%</div><div className="k">{t({ ko: '여정 진행', en: 'Progress' })}</div></div>
          <div className="sl-stat"><div className="v">{points.toLocaleString()}</div><div className="k">{t({ ko: '탐사 포인트', en: 'Points' })}</div></div>
        </div>
      </div>
      ...
      {landmarks && (
        <div className="sl-panel">
          <h3 className="sl-h1" style={{ fontSize: 16 }}>🗺️ {t({ ko: '행성 위치 가이드', en: 'Planet Location Guide' })}</h3>
          <p className="sl-sub" style={{ marginBottom: 10 }}>{t({ ko: '{s}→갈다 직선 위 대략적 위치입니다.', en: 'Approximate spots along the {s}→Galda line.' }, { s: setup.startName })}</p>
          {PLANETS.map((pl) => (
            <div key={pl.key} className={`sl-guide-row ${pl.key === orbit.key ? 'active' : ''}`}>
              <span className="em">{pl.emoji}</span>
              <span className="nm">{pn(pl)}</span>
      ...
      <div className="sl-panel">
        ...
        <p className="sl-hint">
          {mode === 'sim'
            ? t({ ko: '🧪 지도를 클릭해 우주선을 이동시키세요. 동심원은 태양 중심 궤도입니다 — 어느 방향이든 같은 궤도 안이면 미션이 활성화됩니다.', en: '🧪 Click the map to move your ship. The rings are Sun-centered orbits — any direction at the same radius activates that mission.' })
            : `📡 ${t({ ko: '실시간 GPS 추적 중', en: 'Live GPS tracking' })}${accuracy ? ` (±${Math.round(accuracy)}m)` : ''}.`}
        </p>
      </div>

      {/* 궤도 미션 */}
      <div className="sl-panel">
        <h3 className="sl-h1" style={{ fontSize: 16 }}>🛰️ {t({ ko: '궤도 미션', en: 'Orbital Missions' })}</h3>
          ...
          return (
            <div key={pl.key} className={`sl-mission ${pl.key === orbit.key ? 'active' : ''}`}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                <div style={{ flex: 1 }}>
                  <div className="t">{pl.emoji} {pn(pl)} — {mTitle(pl)}{pl.key === orbit.key && ` · ${t({ ko: '현재 궤도', en: 'current orbit' })}`}{pl.axialTilt !== undefined && <span style={{ color: 'var(--sl-muted)', fontWeight: 400 }}> · {t({ ko: '자전축', en: 'tilt' })} {pl.axialTilt}°</span>}</div>
                  <div className="d">{mDesc(pl)}</div>
                </div>
                <button className={`sl-btn ${shot ? '' : 'primary'}`} style={{ padding: '8px 12px', whiteSpace: 'nowrap' }} onClick={() => setCamPlanet(pl)}>
                  {shot ? `✅ ${t({ ko: '재촬영', en: 'Retake' })}` : `📷 ${t({ ko: '촬영', en: 'Shoot' })}`}
                </button>
              </div>
            </div>
          );
      ...
      {camPlanet && (
        <CameraMission
          ...
          onCapture={(url, tilt) => handleCapture(camPlanet, url, tilt)}
        />
      )}
      ...
      {showArrival && (
        <div className="sl-modal-backdrop" onClick={() => setShowArrival(false)}>
          <div className="sl-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <h2>{t({ ko: '갈다(지구)에 도착했습니다!', en: 'You’ve arrived at Galda (Earth)!' })}</h2>
            <p>
              {t({ ko: '{n}, 무사히 1.0 AU 지점에 안착했습니다. 적립 포인트:', en: '{n}, you safely reached the 1.0 AU point. Points earned:' }, { n: setup.nickname })} <strong>{points.toLocaleString()}</strong>
              <br /><br />
              {t({ ko: '벡터를 더 연장하면 ', en: 'Extend the vector further and you’ll find where ' })}<strong>{t({ ko: '태양(0 AU)', en: 'the Sun (0 AU)' })}</strong>{t({ ko: '이 매핑된 지점이 있습니다.', en: ' is mapped.' })}<br />
              {t({ ko: '좌표', en: 'Coords' })}: {journey.sun[0].toFixed(5)}, {journey.sun[1].toFixed(5)} ({t({ ko: '갈다에서 약', en: 'approx.' })} {formatMeters(haversine(GALDA, journey.sun))} {t({ ko: '', en: 'from Galda' })})
            </p>
            <div className="sl-btn-row" style={{ justifyContent: 'center' }}>
              <button className="sl-btn sun" onClick={() => { setToSun(true); setShowArrival(false); }}>☀️ {t({ ko: '태양까지 계속', en: 'Continue to the Sun' })}</button>
              <button className="sl-btn" onClick={() => setShowArrival(false)}>{t({ ko: '여기서 멈춤', en: 'Stop here' })}</button>
            </div>
          </div>
        </div>
      )}
      {toSun && <div className="sl-statusbar" style={{ borderColor: '#ff6b35' }}>{t({ ko: '☀️ 태양을 향해 항행을 계속합니다. 지도의 태양 지점으로 이동해 보세요.', en: '☀️ Sailing on toward the Sun. Try moving to the Sun point on the map.' })}</div>}
    </>
  );
}
